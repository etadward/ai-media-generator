...
- **⚠️ 權衡（實測 2026-06-10）：商品展示廣告 skill 的多 gate agent 規劃很燒 credits**（一支 15s 腕錶廣告，光是表單+方案+分鏡確認的 agent 推理就燒 ~790 credits，影片還沒生）。**要快/省 → 用自由創作手動（test #1 驗證的：Agent▾選模型→Slate注入→send，20cr 跑 Omni 4s）；要引導/精緻/有真實產品圖 → 用 skill。** 自動化跑 skill 要點很多「下一步/確認」button（用 textContent 比對），比手動多 ~8 個 call。
